...
public class BezierCurveUtil {
    ...
    public BezierCurveUtil(RelativeLayout cart, RelativeLayout rl, TextView count, int i) {
        this.cart = cart;
        this.rl = rl;
        this.count = count;
        this.i = i;
     ...
     * @param iv
     */
    public void addCart(final boolean isAdd, ImageView iv) {
//      一、创造出执行动画的主题---imageview
        //代码new一个imageview，图片资源是上面的imageview的图片
        // (这个图片就是执行动画的图片，从开始位置出发，经过一个抛物线（贝塞尔曲线），移动到购物车里)
        final ImageView goods = new ImageView(iv.getContext());
...
//        四、计算中间动画的插值坐标（贝塞尔曲线）（其实就是用贝塞尔曲线来完成起终点的过程）
        ...
        if (isAdd) {
            ...
            path.moveTo(toX, toY);
            //使用二次萨贝尔曲线：注意第一个起始坐标越大，贝塞尔曲线的横向距离就会越大，一般按照下面的式子取即可
            path.quadTo((startX + toX) / 2, toY, startX, startY);
        }
...
//      六、动画结束后的处理
        valueAnimator.addListener(new Animator.AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                cart.startAnimation(animation2);
            }
